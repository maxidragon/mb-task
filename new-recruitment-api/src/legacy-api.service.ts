...
export const createCandidateInLegacyApi = async (payload: LegacyCandidatePayload): Promise<void> => {
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        if (response.status === 409) {
            console.warn(`Attempt ${attempt} failed with 409. Candidate already exists.`);
            return;
        }
        if (!response.ok) {
            throw new Error(`Failed to create candidate in legacy API. Status: ${response.status}`);
        }
    }
