...
export class CandidatesService {
    constructor(private readonly db: Database, private readonly legacyApiService: LegacyApiService) {}

    async create(payload: CreateCandidatePayload) {
        const { firstName, lastName, email, phone, yearsOfExperience, notes, consentDate, jobOfferIds } = payload;
        ...
        try {
            await this.legacyApiService.createCandidateInLegacyApi({ firstName, lastName, email });
        } catch {
        ...
        try {
            const result = await this.db.run(
                `INSERT INTO Candidate (first_name, last_name, email, phone, years_of_experience, notes, consent_date)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [firstName, lastName, email, phone, yearsOfExperience, notes ?? null, consentDate]
            );
